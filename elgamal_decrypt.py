...
def interactive_decrypt():
    ...
    try:
        ...
        print(f"[DEBUG] Finding private key x such that {g}^x mod {p} = {y}")
        ...
        print(f"[DEBUG] Found private key x = {x}")
        print(f"[DEBUG] Verification: {g}^{x} mod {p} = {pow(g, x, p)} (should equal {y})")
